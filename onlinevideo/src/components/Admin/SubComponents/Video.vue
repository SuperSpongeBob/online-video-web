        ...
        <el-collapse v-model="activeName" accordion>
            ...
            <el-collapse-item title="专辑" name="2">
                ...
                <el-table :data="this.albumData" @scroll.capture="handleScroll($event, 'queryAlbum')" border
                    ...
                    <el-table-column prop="videoFavoriteNumber" label="收藏量" width="80" show-overflow-tooltip />
                    ...
                    <el-table-column prop="videoActor" label="演员" show-overflow-tooltip />
                    <el-table-column prop="videoPostPath" label="海报" width="120">
                        <template v-slot="scope">
                            <el-image :src="scope.row.videoPostPath"></el-image>
                        </template>
                    </el-table-column>
                    <el-table-column fixed="right" label="操作" width="100">
                        <template #default="scope">
            ...
            <el-dialog v-model="albumDialog" title="编辑专辑信息" width="500" draggable overflow align-center>
                <el-form ref="dialogAlbumData" :model="dialogAlbumData" label-width="100px">
                    ...
                    <el-form-item label="最新集数">
                        <el-input-number v-model="dialogAlbumData.videoLastUpdate" maxlength="300" placeholder="最新集数"
                            clearable :min="0" />
                    </el-form-item>
                    <el-form-item label="简介">
...
export default {
    data() {
        return {
            ...
            serverAddress: authService.serverAddresss()
        }
    },
    methods: {
        //  根据不同状态显示不同颜色字体
        ...
        async deleteVideo(row) {
            ...
            const response = await authService.deleteVideo(row.videoId)
            if (response.status == 200) {
        ...
        async updateAlbum() {
            const response = await authService.updateAlbum(this.dialogAlbumData)
            if (response.status == 200) {
